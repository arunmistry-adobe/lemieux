/**
 * LeMieux Dashboard – Spend trend panel (rolling weeks, configurable period)
 */
  ...
  try {
    ...
    try {
      ...
      if (u.searchParams.get('dashboardDebugSpend') === '1') {
        console.info(
          '[lemieux-dashboard][SpendTrendDebug]',
          collectSpendTrendDebugSnapshot({
            ordersData,
            spendTrendData,
            resolved,
    ...
    console.warn('lemieux-dashboard: Spend trend render failed:', e);
    if (meta) meta.textContent = 'Weekly breakdown';
    section.appendChild(
      buildEmptyState(
