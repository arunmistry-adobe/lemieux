 ...
 *
 * Main orchestrator for the homepage dashboard experience.
 * Builds the full page shell (left nav + main content), then loads
 * real Commerce data asynchronously to populate each section.
 *
 * Ported from the summit-commerce-b2b `bodea-dashboard` block. The Leaflet
 * map / delivery-sites section from the source is intentionally omitted in
 * this port; the B2B company GraphQL context it relied on lives in
 * `company-context.js`.
 *
 * ── ADDING THIS PAGE ───────────────────────────────────────────────────
 * To place this dashboard on a page:
 * 1. Create a document in Adobe Document Authoring at /dashboard (or /)
 * 2. Add a "LeMieux Dashboard" block to the document (single empty cell)
 ...
 * - company-context.js    — B2B company GraphQL context helpers
 *
 * ── DATA STATUS ────────────────────────────────────────────────────────
 * Real Commerce data: orders list, product names, stock_status
 * Derived from real data: KPI counts
...
import { getDemoDashboardData } from './dashboard-demo-data.js';
...
function buildNewCustomerBanner() {
  ...
  banner.setAttribute('aria-label', 'Welcome to LeMieux');
  ...
  banner.innerHTML = `
    <div class="dashboard-new-customer-banner__inner">
      <div class="dashboard-new-customer-banner__content">
        <h2 class="dashboard-new-customer-banner__heading">Welcome to LeMieux</h2>
        ...
        <div class="dashboard-new-customer-banner__guides">
          ...
          <div class="dashboard-new-customer-banner__guide-btns">
            ...
            <button type="button" class="dashboard-new-customer-banner__guide-btn" disabled>Product Catalogue</button>
          </div>
        </div>
      </div>
  ...
  banner.innerHTML = `
    <div class="dashboard-welcome__text">
      <h1 class="dashboard-welcome__heading">
        ${customerName ? `Welcome back, ${customerName}!` : 'Welcome to LeMieux'}
      </h1>
      <p class="dashboard-welcome__sub">Your equestrian trade hub</p>
    </div>
...
export function buildTopBar(navElement) {
  ...
  topBar.className = 'lemieux-topbar';
  topBar.setAttribute('role', 'banner');

  topBar.innerHTML = `
    <button class="lemieux-topbar__menu-btn" aria-label="Toggle navigation" type="button">
    ...
    <div class="lemieux-topbar__search-wrap">
      <label class="lemieux-topbar__search-label" for="dashboard-search">Search orders</label>
      <div class="lemieux-topbar__search-inner">
        <svg class="lemieux-topbar__search-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <input
          id="dashboard-search"
          type="search"
          class="lemieux-topbar__search-input"
    ...
    <div class="lemieux-topbar__actions">
      <!-- Notifications button + panel -->
      <div class="lemieux-topbar__notif-wrap">
        <button class="lemieux-topbar__icon-btn lemieux-topbar__notif-btn" type="button" aria-label="${unreadCount} notifications" aria-haspopup="true" aria-expanded="false">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/>
            <path d="M13.73 21a2 2 0 0 1-3.46 0"/>
          </svg>
          ${unreadCount > 0 ? `<span class="lemieux-topbar__notif-badge" aria-hidden="true">${unreadCount}</span>` : ''}
        </button>
      </div>

      <!-- Account button + dropdown -->
      <div class="lemieux-topbar__account-wrap">
        <button class="lemieux-topbar__account" type="button" aria-haspopup="true" aria-expanded="false">
          <div class="lemieux-topbar__avatar" aria-hidden="true">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
              <circle cx="12" cy="7" r="4"/>
            </svg>
          </div>
          <div class="lemieux-topbar__account-text">
            <span class="lemieux-topbar__account-name lemieux-topbar__account-name--loading">Loading…</span>
            <span class="lemieux-topbar__account-role">LeMieux trade account</span>
          </div>
          <svg class="lemieux-topbar__account-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
  ...
  topBar.querySelector('.lemieux-topbar__menu-btn').addEventListener('click', () => toggleNav(navElement));

  /* Search */
  const searchInput = topBar.querySelector('.lemieux-topbar__search-input');
  ...
  /* Notification panel */
  const notifWrap = topBar.querySelector('.lemieux-topbar__notif-wrap');
  const notifBtn = topBar.querySelector('.lemieux-topbar__notif-btn');
  ...
  notifBtn.addEventListener('click', (e) => {
    ...
    if (!open) {
      ...
      topBar.querySelector('.lemieux-topbar__account').setAttribute('aria-expanded', 'false');
    }
  });

  /* Account dropdown */
  const accountWrap = topBar.querySelector('.lemieux-topbar__account-wrap');
  const accountBtn = topBar.querySelector('.lemieux-topbar__account');
...
export function updateAccountName(topBar, customerIdentity) {
  const nameEl = topBar.querySelector('.lemieux-topbar__account-name');
  if (!nameEl) return;

  nameEl.classList.remove('lemieux-topbar__account-name--loading');
...
export default function decorate(block) {
  ...
  block.innerHTML = '';
  block.classList.add('lemieux-dashboard');
  ...
  mainEl.className = 'lemieux-dashboard-main';

  const topBar = buildTopBar(nav);
  mainEl.appendChild(topBar);

  const content = document.createElement('div');
  content.className = 'lemieux-dashboard-content';
  ...
  /* ── Demo mode ──────────────────────────────────────────────────────────
   * When there is no authenticated Commerce session (e.g. a sales demo or a
   * logged-out visitor), populate every section with fabricated data so the
   * dashboard looks complete instead of showing "Sign in" empty states.
   * A real signed-in customer always sees live data (below).
   */
  const renderDashboardData = (data, authForRender) => {
    const {
    ...
    updateKpiSection(kpiSection, { ordersData, stockData, isAuthenticated: authForRender });
    updateSpendTrendSection(spendTrendSection, spendTrendData, authForRender, ordersData);
    updateCompanyCreditSection(companyCreditSection, companyCreditData, authForRender);
    updateOrdersSection(ordersSection, ordersData, authForRender);
  ...
  const loadDashboardData = async () => {
    if (!isAuthenticated) {
      /* Render demo data as if authenticated so all panels populate. */
      renderDashboardData(getDemoDashboardData(), true);
      return;
    }
    ...
    try {
      ...
      console.error('[LeMieux Dashboard] Data load failed:', err);
